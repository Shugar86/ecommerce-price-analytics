...
# Воркер аналитики: интервал; демо-история цен для графиков; порог кандидатов TF‑IDF (EKF↔TDM)
AI_WORKER_INTERVAL_SEC=300
SEED_DEMO_HISTORY=1
# AI_MATCH_MIN_SCORE=0.45

# Полный список переменных: env.example. Объём «сопоставления» (кандидаты vs exact): docs/PRODUCT_SCOPE.md
```

После запуска откройте веб-интерфейс аналитика: **http://localhost:8000** (сервис `web`). Раздел **«Сопоставления»** — кандидаты по сходству наименований; ревью (подтвердить/отклонить) в той же таблице.
